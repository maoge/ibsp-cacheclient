...
public class CommandArgs<K, V> {
    ...
    private Collection<byte[]> argsColl;
    ...
    public CommandArgs(RedisCodec<K, V> codec) {
        this.codec  = codec;
        this.argsColl = new ArrayList<byte[]>();
    ...
    public CommandArgs<K, V> write(byte[]... args) {
        for(byte[] arg : args) {
        	write(arg);
        }
        return this;
    ...
    private CommandArgs<K, V> write(String arg) {   	
    	try {
			return write(arg.getBytes("UTF-8"));
    ...
    public Collection<byte[]> getArgs() {
